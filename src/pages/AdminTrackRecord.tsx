...
export default function AdminTrackRecord({ params }: { params: { phone: string } }) {
  ...
  const dietPlanStatus = (record: AssessmentData) => {
    const MEAL_KEYS = [
      ...
      "eveningSnack", "preWorkout", "postWorkout", "dinner", "beforeBed",
      "supplementsPlan", "notes",
    ];
    const filled = MEAL_KEYS.filter((k) => !!(record as Record<string, unknown>)[k]);
            ...
            <>
              ...
              <div className="space-y-4">
                {records.map((record, idx) => {
                  ...
                  return (
                    <motion.div
                      key={record.id ?? record._arrayIndex}
